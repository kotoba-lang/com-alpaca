(ns alpaca.main
  "Kotodama WASM entrypoint for the Alpaca clean-room actor (L4) — Clojure port.
  ...
  facts (`alpaca.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "alpaca")
(def tier "L4")
...
(def entity-specs
  [{:entity "Asset"        :plural "assets"         :id-prefix "alpaca_ass"
    :fields [:symbol :assetClass :exchange :status :tradable :fractionable] :required [:symbol :assetClass]
    :coerce {:tradable :bool :fractionable :bool} :refs {}}
   {:entity "Order"        :plural "orders"         :id-prefix "alpaca_ord"
    :fields [:symbol :side :type :status :qty :timeInForce] :required [:symbol :side]
    :coerce {}                                       :refs {}}
   {:entity "Position"     :plural "positions"      :id-prefix "alpaca_pos"
    :fields [:symbol :qty :side :avgEntryPrice :marketValue] :required [:symbol :qty]
    :coerce {}                                       :refs {}}
   {:entity "TradeAccount" :plural "tradeaccounts"  :id-prefix "alpaca_tra"
    :fields [:accountNumber :status :equity :cash :buyingPower :patternDayTrader] :required [:accountNumber :status]
    :coerce {:patternDayTrader :bool}                :refs {}}
   {:entity "TradeActivity" :plural "tradeactivities" :id-prefix "alpaca_tra"
    :fields [:symbol :side :price :qty :type :orderId] :required [:symbol :side]
    :coerce {:price :float :qty :float}              :refs {:orderId "Order"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"alpaca.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "alpaca-compat" :tier tier :entities entities} 200])
...
(defn start! [] :alpaca-compat/ready)
